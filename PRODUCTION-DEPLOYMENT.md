# 🚀 Production Deployment Guide - kaiyuutcg.com.br

## 📋 **Final URL Structure**
After deployment, your TCG store will be accessible at:

- **🛍️ Main Store**: `https://kaiyuutcg.com.br`
- **⚙️ Admin Panel**: `https://api.kaiyuutcg.com.br/admin`
- **📡 API Endpoint**: `https://api.kaiyuutcg.com.br/api`
- **🪝 Asaas Webhook**: `https://api.kaiyuutcg.com.br/api/payments/webhook`
...
gcloud compute instances create kaiyuu-tcg-prod \
  --zone=us-central1-a \
...
gcloud compute firewall-rules create kaiyuu-web-ports \
  --allow tcp:80,tcp:443,tcp:3000,tcp:1337 \
  --source-ranges 0.0.0.0/0 \
  --description "Allow web traffic for Kaiyuu TCG"

# Get external IP
gcloud compute instances describe kaiyuu-tcg-prod \
  --zone=us-central1-a \
...
```bash
# SSH into VM
gcloud compute ssh kaiyuu-tcg-prod --zone=us-central1-a
...
./deploy-docker.sh kaiyuutcg.com.br YOUR_ASAAS_PRODUCTION_API_KEY
...
3. **Add Webhook URL**: `https://api.kaiyuutcg.com.br/api/payments/webhook`
4. **Enable Events**:
   - ✅ Payment received
   - ✅ Payment confirmed  
   - ✅ Payment overdue
...
docker exec -it kaiyuu-backend-prod bash

# Create admin user
npm run strapi admin:create-user -- \
  --firstname="Admin" \
  --lastname="Kaiyuu" \
  --email="admin@kaiyuutcg.com.br" \
...
### **Frontend (Store) - https://kaiyuutcg.com.br**
...
### **Backend (API) - https://api.kaiyuutcg.com.br**
...
```bash
# Create backup
docker exec kaiyuu-db-prod pg_dump -U kaiyuu_user kaiyuu_tcg > backup_$(date +%Y%m%d).sql

# Restore backup
cat backup_20240101.sql | docker exec -i kaiyuu-db-prod psql -U kaiyuu_user kaiyuu_tcg
```

### **SSL Certificate Renewal (if not using Cloudflare)**
```bash
# Using Let's Encrypt (if you decide not to use Cloudflare)
certbot --nginx -d kaiyuutcg.com.br -d api.kaiyuutcg.com.br
```
...
2. **Test thoroughly** on https://kaiyuutcg.com.br ✅
...
docker exec kaiyuu-db-prod psql -U kaiyuu_user -d kaiyuu_tcg -c "SELECT version();"
```

### **Payment issues**
1. Verify Asaas API key is production key
2. Check webhook URL in Asaas dashboard
3. Test webhook: `curl -X POST https://api.kaiyuutcg.com.br/api/payments/webhook`

---

## 📞 **Support**

Your production TCG store will be live at **https://kaiyuutcg.com.br** with professional SSL, fast CDN, and payment processing ready! 🎉
